...
for($prof;$prof>-1;$prof--)
{
	...
	if ($handle)
	{
		...
		while ($ligne = fgetcsv($handle, $taille, $delimiteur)) 
		{
			if($Debut==0)
			{
				$Debut=strtotime($ligne[1]." ".$ligne[2]);
			}
			$index=floor((strtotime($ligne[1]." ".$ligne[2])-$Debut)/288);
			
			if($debug==1)
				echo $index." ".$ligne[1]." ".$ligne[2];
			...
			switch ($ligne[0])
			{
				...
				case "192.168.0.0":
					...
					{
						$DateSalon[$index] 	= $ligne[1];
						$TimeSalon[$index] 	= $ligne[2];					
						$TSalon[$index] 	= $ligne[3]+$calSalon;
						$Pression[$index] 	= $ligne[4];
					}
				break;
				...
				case "192.168.0.1":
					...
					{
						$DateVeranda[$index] 	= $ligne[1];
						$TimeVeranda[$index] 	= $ligne[2];
						$TVeranda[$index] 	= $ligne[3]+$calVeranda;
					}
				break;
				...
				case "192.168.0.3":
					...
					if(is_numeric ($ligne[3]))
					{
						$DateExt[$index] 	= $ligne[1];
						$TimeExt[$index] 	= $ligne[2];
						$TExt[$index] 	= $ligne[3]+$calExt;
					}
				break;
				default :
					#echo "erreur" .$ligne[0];
